...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, member }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-80 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-lg bg-white rounded-2xl overflow-hidden shadow-2xl transform transition-all" onClick={e => e.stopPropagation()}>
                <button
                    onClick={onClose}
                    ...
                    <div className="w-full md:w-3/5 p-6 flex flex-col justify-center">
                        ...
                        <div className="flex space-x-4">
                            {member.social.facebook && (
                                <a href={member.social.facebook} className="text-gray-400 hover:text-blue-700 transition-colors" title="Facebook">
                                    <Facebook size={28} />
                                </a>
                            )}
                            {member.social.instagram && (
                                <a href={member.social.instagram} className="text-gray-400 hover:text-pink-600 transition-colors" title="Instagram">
                                    <Instagram size={28} />
                                </a>
                            )}
                        </div>
